  ...
  <div class="settings screen">
    ...
    <section class="settings__section section">
      <div class="flex-between">
        ...
        <button class="add-btn" @click="showAddCardModal = true">+ เพิ่ม</button>
      </div>
    ...
    <section class="settings__section section">
      <div class="flex-between">
        ...
        <button class="add-btn" @click="showAddBankModal = true">+ เพิ่ม</button>
      </div>
      
      <div v-if="paymentStore.banks.length > 0" class="banks-list">
        <JCard v-for="bank in paymentStore.banks" :key="bank.bankId">
          <div class="bank-item">
            <div class="bank-item__logo" :style="{ background: getBankColor(bank.bankCode) }">
    ...
    <div v-if="showAddCardModal" class="modal-overlay" @click.self="showAddCardModal = false">
      <div class="modal">
        ...
        <div class="modal__body">
          <JInput v-model="newCard.number" label="หมายเลขบัตร" placeholder="1234 5678 9012 3456" maxlength="19" />
          <div class="modal__row">
            <JInput v-model="newCard.expiry" label="วันหมดอายุ" placeholder="MM/YY" maxlength="5" />
            <JInput v-model="newCard.cvv" label="CVV" placeholder="123" maxlength="3" type="password" />
          </div>
          <JInput v-model="newCard.name" label="ชื่อบนบัตร" placeholder="SOMCHAI T" />
        </div>
      ...
      <div class="modal">
        ...
        <div class="modal__body">
          ...
          <div class="banks-grid">
            ...
            >
              <div class="bank-option__logo" :style="{ background: bank.color }">{{ bank.code.slice(0, 1) }}</div>
              <span class="bank-option__name">{{ bank.name }}</span>
            </button>
          </div>
        </div>
        <div class="modal__footer">
          <JButton variant="primary" :disabled="!selectedBankCode" @click="addNewBank">ดำเนินการต่อ</JButton>
        </div>
      </div>
    </div>
  </div>
...
const removeCard = (cardId) => {
  if (confirm('ต้องการลบบัตรนี้?')) {
    paymentStore.removeCard(cardId)
  }
}

const removeBank = (bankId) => {
  if (confirm('ต้องการลบบัญชีนี้?')) {
    paymentStore.removeBank(bankId)
  }
}
